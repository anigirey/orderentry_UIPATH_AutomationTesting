...
def main():
    ...
    sanityWorkbook = xlsxwriter.Workbook(str(testEnv+'_SR.xlsx'))
    sanityWorksheet = sanityWorkbook.add_worksheet(testTim)
    sanityWorkbook.close()
    
    sanityWorkbook = openpyxl.load_workbook(str(testEnv+'_SR.xlsx'))
    sanityWorksheet = sanityWorkbook.get_sheet_by_name(testTim)
