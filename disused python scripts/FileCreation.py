...
def main():
    ...
    testEnv = ["TEST1", "TEST2", "TEST3", "E2E"]
    testCase = ["1FR", "1FB", "HSI", "PRISM", "DHP"]
    now = datetime.now()
    ...
    # creates a .xlsx file that records only the time and date for processing
    # filename is */Results/outputfile.xlsx
    if not path.exists(str(currDir) + "/Output/"):
        outputDirectory = (str(currDir) + "/Output/")
        os.makedirs(outputDirectory)
    outputWorkbook = xlsxwriter.Workbook(str(currDir) + "/Output/outputfile.xlsx")
    outputWorksheet = outputWorkbook.add_worksheet("Output")
    outputWorksheet.set_column("A:A", 10)
    outputWorksheet.set_column("B:B", 10)
    outputWorksheet.write("A1", rDateStr)
    outputWorksheet.write("B1", rTimeStr)
